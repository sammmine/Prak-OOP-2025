...
public class Penyihir implements Kelas {
    ...
    public void gunakanKemampuan(Karakter self, Karakter target) {
        if (target.isHidup()) {
            int manaDiserap = Math.min(40, target.getMana());
            target.reduceMana(manaDiserap);
            self.addMana(manaDiserap);
            System.out.println(self.getName() + " menyerap " + manaDiserap + " mana dari " + target.getName());
    ...
    public void gunakanUltimate(Karakter self, Karakter target) {
        if (target.isHidup()) {
            ...
            System.out.println(target.getName() + " sudah mati, tidak bisa disihir");
        }
    }
